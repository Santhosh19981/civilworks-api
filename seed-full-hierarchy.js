...
async function run() {
  ...
  try {
    ...
    try {
      await connection.query('ALTER TABLE categories ADD COLUMN parent_id INT NULL AFTER id');
    ...
    try {
        await connection.query('ALTER TABLE categories ADD CONSTRAINT fk_category_parent FOREIGN KEY (parent_id) REFERENCES categories(id) ON DELETE CASCADE');
        console.log("FK constraint added.");
    } catch(e) {
        console.log("FK constraint might already exist.");
    }
    ...
    for (const cat of data) {
      ...
      for (const sub of cat.subcategories) {
        const [subResult] = await connection.query('INSERT INTO categories (name, parent_id, icon) VALUES (?, ?, ?)', [sub.name, catId, 'subcategory']);
        ...
        for (const prod of sub.products) {
          ...
          await connection.query(
            ...
            [subId, prod, slug, 100.00, `Description for ${prod}`]
          );
        }
      }
    }
